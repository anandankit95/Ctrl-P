...
procdump(void)
{
  ...
  char *state;
  //uint pc[10];
	cprintf("Name \t P_id \t State \t Time Elapsed \n");
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->state == UNUSED)
      continue;
    ...
    cprintf("%s\t %d\t %s\t %d.%d", p->name, p->pid,state,(curr_time-p->start_time)/100,(curr_time-p->start_time)%100);
...
//Print all the Process Status
//Print all the Process ID
//Print all the Process Uptime
int
ps()
{
	int curr_time=0;
